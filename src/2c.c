...
// static const char *determine_bits_field(unsigned length)
// {
// 	const char *field = "128";
// 	sprintf(field, "%d", length);
// 	return field;
// }
...
static int signal2type(signal_t *sig, FILE *o)
{
	...
	printf("name is %s, start bit %d, bit length: %d\n", sig->name, sig->start_bit, sig->bit_length);
	if (sig->comment) {
		...
		return fprintf(o, "\t%s %s : %d; /* scaling %.1f, offset %.1f, units %s %s */\n",
				type, sig->name, length, sig->scaling, sig->offset, sig->units[0] ? sig->units : "none",
				sig->is_floating ? ", floating" : "");
	}
}
...
static void make_name(char *newname, size_t maxlen, const char *name, unsigned id)
{
	assert(newname);
	assert(name);
	snprintf(newname, maxlen-1, "%s", name);
}
...
static int msg2h_types(dbc_t *dbc, FILE *h) 
{
	...
	for (size_t i = 0; i < dbc->message_count; i++) {
		can_msg_t *msg = dbc->messages[i];
		char name[MAX_NAME_LENGTH] = {0};
		// function to generate struct name
		make_name(name, MAX_NAME_LENGTH, msg->name, msg->id);
		printf("%s\n", name);
		if (msg->comment)
			fprintf(h, "/* %s */\n", msg->comment);
